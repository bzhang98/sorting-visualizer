import quickSortInPlace from "../../src/sorting_functions/quick-sort-inplace";

describe("Quick Sort", () => {
  it("should return an empty array when an empty array is passed", () => {
    expect(quickSortInPlace([])).toEqual([]);
  });

  it("should return the same array when it has only one element", () => {
    expect(quickSortInPlace([1])).toEqual([1]);
  });

  it("should return the array sorted in ascending order", () => {
    expect(quickSortInPlace([5, 4, 3, 2, 1])).toEqual([1, 2, 3, 4, 5]);
  });

  it("should work with arrays that have repeated elements", () => {
    expect(quickSortInPlace([3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5])).toEqual([
  ...
  it("should sort a large array correctly", () => {
    ...
    expect(quickSortInPlace(largeArray)).toEqual(sortedArray);
  });
